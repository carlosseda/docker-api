class AdminFilter extends HTMLElement {
  ...
  render () {
    ...
    for (const tab in this.structure.tabs) {
      ...
      for (const row in this.structure.tabsContent[tab].rows) {
        ...
        for (const field in this.structure.tabsContent[tab].rows[row].formElements) {
          const formElement = this.structure.tabsContent[tab].rows[row].formElements[field]
  ...
  renderButtons () {
    ...
    applyFilterButton.addEventListener('click', async () => {
      ...
      const url = formDataJson.prompt ? `${window.env.API_URL}${this.getAttribute('endpoint')}/openai-filter` : `${window.env.API_URL}${this.getAttribute('endpoint')}?${queryString}`
      const fetchOptions = {
        method: method,
        headers: {
      ...
      try{
        const response = await window.axios(url, fetchOptions)
...
customElements.define('admin-filter-component', AdminFilter)
