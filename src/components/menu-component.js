class Menu extends HTMLElement {
  ...
  connectedCallback () {
    document.addEventListener('DOMContentLoaded', () => {
      this.component = window.env.STRUCTURE.loadComponents['menu-component'][this.getAttribute('endpoint')][this.getAttribute('method')]
      this.render()
    })
  }
  ...
  render () {
    ...
    Object.values(this.component.data).forEach(menuItem => {
      ...
      link.textContent = menuItem.locales[0].value
  ...
  createSubMenu (menuItem, li) {
    if (menuItem.children) {
      ...
      Object.values(menuItem.children).forEach(subMenuItem => {
        ...
        if (menuItem.localeSeo.url) 
          subLink.setAttribute('href', `${subMenuItem.localeSeo.url}`)
