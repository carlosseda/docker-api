class TrackingComponent extends HTMLElement {
  ...
  constructor() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      window.axios.post(`${window.env.API_URL}user-trackings`,
        { data: data }
      )
    })
  }
  ...
  generateFingerprint = () => {
    ...
    const userAgent = window.bowser.parse(window.navigator.userAgent)
    ...
    window.axios.post(`${window.env.API_URL}fingerprints`,
      { data: data }
    )

    this.dispatchEvent(new CustomEvent('fingerprint', {
      detail: {
        fingerprint: this.fingerprint
      }
    }))
